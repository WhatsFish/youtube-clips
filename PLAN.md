...
- **YouTube cookies need manual refresh per download burst.** YouTube's anti-bot is much more aggressive against Azure datacenter IPs than against residential ones. Even freshly-exported, valid cookies (containing `__Secure-3PSID` etc.) get rejected within hours — sometimes within minutes — when used from this VM. Symptom: yt-dlp returns `Sign in to confirm you're not a bot`. **First-pass mitigation deployed 2026-05-09**: a `bgutil-ytdlp-pot-provider` docker service mints YouTube PO tokens out-of-browser; paired with the pip plugin in `.venv` it auto-attaches a valid attestation token to each request. Doesn't fix the IP reputation problem but should shift cookie longevity from hours → days. **In observation now** — track how often manual refresh is still necessary. If still <1 day, escalate to paid residential proxy (the home network is in mainland China and routes through this same VM, so DIY residential routing isn't an option here). Workaround when it does fail: re-export `~/.config/youtube-clips-cookies.txt` from a logged-in browser. This is still the blocker for putting downloads under cron.
...
| YouTube anti-bot from Azure datacenter IPs | 🧪 experiment in observation (2026-05-09) | First-pass mitigation deployed: a `bgutil-ytdlp-pot-provider` docker service that mints valid YouTube PO (proof-of-origin) tokens out-of-browser, paired with the matching pip plugin in `.venv`. Hypothesis: with PO token attached, fresh cookies survive longer before YouTube rotates them. Won't change the IP reputation problem itself — that's hard-coded into the IP block — but should shift the failure mode from "hours" toward "days." **Observe how often manual cookie refresh becomes necessary now.** If still <1 day, escalate to the original three real fixes: (a) residential proxy via `--proxy`, (b) download from a home machine and rsync to VM, (c) VPN to a residential network. Note: home network is in mainland China and reverse-proxies through this VM, so options (b)/(c) don't have a clean residential IP to draw on — paid residential proxy (option a) becomes the only escalation path if PO token isn't enough. |
