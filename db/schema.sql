...
-- ============================================================
-- Phase 2 only renders one platform variant (Bilibili long, 16:9), so the
-- seed reflects that scope. Phase 3 will extend output.platforms in this
-- row's config_jsonb (or replace via UPDATE) without touching the schema.
...
VALUES (
  ...
  '{
    ...
    "topic_generation_prompt": "你为一个面向中文受众的科技频道选题。候选话题应当聚焦最近 1-2 周内英文科技 YouTube 上有讨论度的内容（AI、开发者工具、新品发布、行业动态），适合做 5-10 分钟解说视频。每次产出 5-10 个候选 topic，每个含中文标题、一段中文描述和 3-5 个英文搜索关键词。",
    "edit_style_prompt": "你写 commentary 风格的剪辑决策列表：从英文素材里挑出最有信息量的 5-10 个片段（每段 8-25 秒），用中文重新串讲。原片音轨压低做衬底（ducked_original），中文配音做主轨。语气专业但不刻板，可以有观点。"
  }'::jsonb,
  TRUE
)
