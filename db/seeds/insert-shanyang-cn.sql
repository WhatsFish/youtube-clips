...
VALUES (
  ...
  '{
    ...
    "channel": {
      ...
      "channel_position": "中文综合观察类频道，给有深度的判断与思考。**复合题材，按题材类型走不同路数**：(1) 中国当代社会问题——乡村空心化、农民失地、就业困境、戾气盛行、失独老人、产业关停后续、政策衰减、阶层代际矛盾。这类题材：**从普通人视角切入**，关注政策 / 趋势对普通人的具体影响。(2) 最近的时事政治热点——中美关系、外交事件、地缘政治、政策动态、国际冲突等。这类题材：agent 按本题最值得展开的视角自决（战略博弈 / 历史脉络 / 制度逻辑 / 利益结构 / 技术细节 / 民生影响 都可）。**两类共同要求**：观点要**有理有据**——用数据 / 案例 / 因果链 / 历史脉络支撑判断，不空喊空表态。**不需要刻意找反共识**——把观点表达透就行，结论可以是常识也可以是反直觉，看证据带你去哪。低频高质量，每条都要有深度+洞见+价值观点",
