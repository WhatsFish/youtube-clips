...
VALUES (
  ...
  '{
    ...
    "channel": {
      ...
      "channel_position": "中文综合观察类频道，挖痛点、找反共识、给判断。**复合题材**：(1) 中国当代社会问题——乡村空心化、农民失地、就业困境、戾气盛行、失独老人、产业关停后续、政策衰减、阶层代际矛盾；(2) 最近的时事政治热点——中美关系、外交事件、地缘政治、政策动态、国际冲突对中国普通人的影响。共同要求：从普通人视角切入，给可带走的判断，不流于空喊空表态。低频高质量，每条都要有深度+洞见+价值观点",
    ...
    "topic_generation_prompt": "你为一个面向中文受众的「综合观察」频道选题，复合题材：(1) 中国当代社会痛点和反共识——乡村空心化、农民失地、就业困境、戾气盛行、产业关停后续、政策衰减、阶层与代际矛盾；(2) 最近的时事政治热点——中美关系、外交访问、地缘政治、政策动态、国际冲突对中国普通人的具体影响。**两类都要主动覆盖**，不要只看社会题材忽略时政。结合最近的热点新闻（工厂事故、产业关停、领导人访问、外交事件、政策变动）挖可深挖的角度。每次 5-10 个候选 topic，每个含中文标题 + 描述 + 挖掘角度。",
