...
name: edl-continuous
version: 8
purpose: Stage 2 — agent 全权决定节奏 + shot 顺序，不再硬约束 monotone
...
required_placeholders:
  ...
  - analysis_block
notes: |
  v6: 固定 shot 数和字数；vlog 太密、verbal_tics 强行塞。
  v7: 节奏交给 agent（pacing tier），但 shot 顺序锁成"必须 monotonic、
      最多 1 处倒叙 hook"——这个约束把好的叙事结构（多次倒叙、callback、
      对比剪辑）也禁了。
  v8: shot 顺序也交给 agent。可严格按源时间，也可倒叙、callback、多线
      穿插，只要每次跳跃都有叙事意义。约束改成"原则性"而非"机械性"。
---
...
**重要：**
- **shot 顺序由你决定**。可以严格按源视频时间走，也可以倒叙、callback、多线穿插、对比剪辑。**唯一要求：每一次跳跃都要有叙事意义**（hook、回扣、设悬念、对比强化），不要为跳而跳让观众迷路。如果用复杂结构（多次倒叙 / 时间穿插），shot 之间的 narration 要给观众过渡线索（「先看后面发生了什么」「现在回到那个…」「同一时间另一边…」之类的桥句）
- 收尾要有"句号感" —— 最后一个 shot 不要戛然而止，应该是结论性的、留余韵的一句，让观众觉得「这就完了」而不是「视频卡住了」。renderer 会在最后给一个画面 + 音频淡出，但 narration 本身的文气也要收得住。
- verbal_tics 自然处用，**整篇最多 2 处**，不重复
- 所有 narration 拼起来读出来必须像**一个真人在轻松讲话**，不是机器朗读
- 不要把画面里观众自己看得到的事再讲一遍，**加一层信息**
- **第一遍听懂原则**：每句 narration 必须是中国观众**第一遍听就能立刻理解**的中文。**禁止隐喻 / 抽象修辞**让观众反向解析（例如「传统被她切成五分钟一段」这种：第一遍听就糊涂）。要么换成大白话直说，要么这一层意思干脆别提。**写完每句自己读一遍 / 想一遍：路人能秒懂吗？不能就改。**
- **TTS 兼容原则**：narration 字段会被 Azure 中文 TTS 朗读。**禁止出现非中文字符（日文假名 / 韩文 / 西里尔文 / 阿拉伯文 / 大段英文单词）**——TTS 读不出来会直接跳过或乱念。要提及外文专名时：
  - 日语「ごちそうさま」→ 写成「她合掌说了一句谢饭礼」「说一句日语，意思是『我吃饱了』」
  - 英语品牌名简单的可以保留（YouTube、Apple），长句子必须意译
  - 韩文 / 法文 / 德文 / 任何非英文外语，**100% 用中文意译/音译**，原文只能放在 description_zh 或 tags 里，不能进 narration
- **画面准确性原则**：如果你不能 100% 确定某个画面元素出现在哪个具体时间戳，**别在 narration 里指认它**。Stage 1 的 evidence 已经标过时间，**严格用那些时间戳**，不要自己外推。如果想说一个 evidence 没明确支持的细节，要么省略，要么用更模糊的表达（"看上去""估计""家里那台"），不要说得像在指证那一秒的画面。
