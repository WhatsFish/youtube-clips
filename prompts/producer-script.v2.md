...
- `"image"`（**静态画面首选**）—— CogView 文生图 + ken-burns 推拉。免费、~10s 出图、质量好。右下「AI 生成」小水印。**适合本身没什么运动的画面**：文档、建筑、静物、抽象概念、人物剪影。
- `"ai"`（**真实运动需求**）—— Doubao Seedance 文生视频，~$0.06/5s clip。**只在画面真正需要运动时用**：人在走 / 车流 / 风吹动 / 工人作业 / 街道人来人往。ai 也不贵了但比 image 慢 + 花钱，**ROI 不够就别上**。
...
- 这个画面**有没有具体的运动**对叙事重要？  
  ...
  → 有 → 进入下一题  
- 这个运动 **Pexels 大概率能搜到**？  
  → 能（一般场景：城市街景 / 打字 / 吃饭）→ `pexels`  
  → 不能（中国具体：县城工地 / 农村集市 / 招工启事）→ `ai`  
- 这个画面 **是中国文化具体场景**（不需要运动）？  
  → 是 → `image`（用 ken-burns 弥补不动感）  
  → 否 → `pexels`

整支视频不要被 `ai` / `image` 比例公式绑死。**根据每个 shot 实际需求**自决。但**一条视频里大量重复 `image`（>60%）会让观众觉得 PPT 化**——尽量让动静画面互相穿插。
